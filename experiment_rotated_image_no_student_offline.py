...
from utils import test_model_with_rejection, get_validation_proba
...
student_optimizer_scheduler = OneCycleLR(student_optimizer, max_lr=0.1,
                                         total_steps=(len(
                                             train_loader) * args.num_distillation_epochs) + 1)
...
for epoch in tqdm(range(int(args.num_distillation_epochs))):
    ...
    if t > args.burn_in_iters:
        if epoch % 5 == 0:
            student_data_ensemble_proba.append(get_validation_proba(teacher_model, student_train_data, 256))
            _, teacher_val_accuracy, teacher_val_proba, _, _, _ = \
                test_model_with_rejection(teacher_model, test_loader, cost_matrix, num_classes)
            teacher_model_ensemble_proba.append(teacher_val_proba)
            print("Epoch: %d, teacher test accuracy: %f \n" % (
                epoch, teacher_val_accuracy))

student_data_ensemble_proba = torch.stack(student_data_ensemble_proba).mean(dim=0)
...
for epoch in tqdm(range(int(args.num_distillation_epochs))):
    for data, labels in train_loader:
        batch_idxs = torch.randperm(len(student_train_labels))[:args.batch_size]
        batch_data = student_train_data[batch_idxs].to(args.device)
        student_model_batch_logits = student_model(batch_data)
        teacher_proba = student_data_ensemble_proba[batch_idxs]
        ce_loss = -torch.sum(F.log_softmax(student_model_batch_logits, dim=-1).exp() *
                             torch.log(teacher_proba), dim=1).mean()
