...
student_train_loader = DataLoader(deepcopy(train_set), batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                          pin_memory=True)
...
if args.pretrain_epochs is not None:
    ...
    pretrain_optimizer_scheduler = OneCycleLR(pretrain_optimizer, max_lr=0.05, epochs=args.pretrain_epochs, steps_per_epoch=len(train_loader))
...
for epoch in tqdm(range(int(args.num_distillation_epochs))):
    ...
    for batch_idx, (batch_data, batch_labels) in enumerate(train_loader):
        ...
        if t > args.burn_in_iters:
            ...
            student_argmaxed_decision_cost_mean = (student_decision_cost * student_decision_softmax).sum(
                dim=-1).mean()
            teacher_model_batch_proba = F.log_softmax(teacher_model(batch_data) / args.temp, dim=-1).exp()
            student_loss = -torch.sum(teacher_model_batch_proba *
                                      F.log_softmax(student_model_batch_logits / args.temp, dim=-1), dim=1).mean() + \
...
# val_labels = torch.tensor(test_set.targets).long().squeeze()
# ood_preds = torch.zeros_like(val_labels)
# ood_preds[(teacher_model_ensemble_preds == num_classes).nonzero().squeeze()] = 1
# ood_labels = torch.zeros_like(val_labels)
# ood_labels[(val_labels == num_classes).nonzero().squeeze()] = 1
# teacher_model_ensemble_ood_precision_score = precision_score(ood_labels.numpy().squeeze(), ood_preds.numpy().squeeze())
# teacher_model_ensemble_ood_recall_score = recall_score(ood_labels.numpy().squeeze(), ood_preds.numpy().squeeze())
# teacher_model_ensemble_loss = F.nll_loss(teacher_model_ensemble_proba.log()[non_rejection_indices],
#                                          test_set.targets[non_rejection_indices].to(args.device))
# teacher_model_ensemble_ece = get_ece(teacher_model_ensemble_proba[non_rejection_indices].cpu().numpy(),
#                                      test_set.targets[non_rejection_indices].cpu().numpy())
# teacher_model_ensemble_auroc = roc_auc_score(test_set.targets[non_rejection_indices].cpu().numpy(),
#                                              teacher_model_ensemble_proba[non_rejection_indices].cpu().numpy()[:, 1])
# teacher_model_ensemble_ap_score = average_precision_score(test_set.targets[non_rejection_indices].cpu().numpy(),
#                                                           teacher_model_ensemble_proba[
#                                                               non_rejection_indices].cpu().numpy()[:, 1])
# output_results_list += [teacher_model_ensemble_loss.cpu().item(), teacher_model_ensemble_accuracy,
#                         teacher_model_ensemble_decision_cost_mean, teacher_model_ensemble_ece,
#                         teacher_model_ensemble_auroc, teacher_model_ensemble_ap_score, num_rejected_samples]
...
if not args.run_baseline_only:
    ...
    for epoch in tqdm(range(int(args.num_epochs_q_training))):
        ...
        total_epoch_ce_loss = 0.
        total_epoch_decision_cost = 0.
        for batch_idx, (batch_data, batch_labels) in enumerate(student_train_loader):
            batch_data = batch_data.to(args.device)
            batch_labels = batch_labels.to(args.device)
            batch_data += torch.randn_like(batch_data, requires_grad=False) * args.data_noise
            ...
            total_epoch_ce_loss += (ce_loss - entropy_loss).cpu().item() * len(batch_data)
            total_epoch_decision_cost += argmaxed_decision_cost_mean.cpu().item() * len(batch_data)
        avg_epoch_ce_loss = total_epoch_ce_loss / len(train_loader.dataset)
        avg_epoch_decision_cost = total_epoch_decision_cost / len(train_loader.dataset)
        ...
        if epoch % 5 == 0:
            _, val_accuracy, val_proba, _, _, _ = test_model_with_rejection(q_model,
                                                                            test_loader,
                                                                            cost_matrix,
                                                                            num_classes)
            print("Epoch: %d,  avg accuracy: %f" % (epoch, val_accuracy))

    val_loss, val_accuracy, val_proba, num_rejected_samples, rejection_indices, non_rejection_indices = \
        test_model_with_rejection(q_model,
                                  test_loader,
                                  cost_matrix,
                                  num_classes)
    ...
    ##TODO: Give meaningful names below
    A = np.expand_dims(torch.matmul(q_model_decision_softmax, test_cost_matrix.transpose(0, 1)).cpu().numpy(),
                       1)
    B = np.expand_dims(
        F.one_hot(torch.tensor(test_set.targets).long().squeeze(), num_classes=num_classes + 1).cpu().numpy().T, 0)
    B = np.matmul(A, B.T)
    ...
    # q_model_ece = get_ece(val_proba.cpu().numpy()[non_rejection_indices],
    #                       test_set.targets.cpu().numpy()[non_rejection_indices])
    # q_model_auroc = roc_auc_score(test_set.targets.cpu().numpy()[non_rejection_indices],
    #                               val_proba.cpu().numpy()[non_rejection_indices][:, 1])
    # q_model_ap_score = average_precision_score(test_set.targets.cpu().numpy()[non_rejection_indices],
    #                                            val_proba.cpu().numpy()[non_rejection_indices][:, 1])
    output_results_list += [val_loss.cpu().item(), val_accuracy, q_model_decision_cost_mean, num_rejected_samples]
